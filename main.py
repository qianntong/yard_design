...
def process_train_data(departure_file, yard_plan_file, output_dir):
    ...
    for _, row in dep_df.iterrows():
        ...
        # === Step 3: 在 yard_plan Sheet2 中查找该 Train ===
        matched_rows = yard_sheet2[yard_sheet2.astype(str).apply(lambda x: x.str.contains(train_name)).any(axis=1)]
        if matched_rows.empty:
            print(f"⚠️ No match found for train {train_name} in yard_plan Sheet2.")
            continue

        # 找到 block 列（第一行标识 block 名）
        ...
        if not block_columns:
            print(f"⚠️ No matching blocks {blocks} found in yard_plan for {train_name}.")
            continue

        # === Step 4: 对每个小时计算 CAR ARRIVING ===
        hours = list(range(0, 24))
        car_arriving_per_hour = []
        for h in hours:
            # 假设 yard_plan 每小时数据存在某列或索引
        ...
        # === Step 5: 聚合计算 total_car, dwell_hours, total_car_hours, car_hours ===
        ...
        # === Step 6: 反向计算 car_hours ===
        car_hours = []
        for i in reversed(range(24)):  # 从23开始倒推
            hour = hours[i]
